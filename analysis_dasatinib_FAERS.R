...
# The file 'aersMineExploreDataSet_4372 additional renal ADRS.aers' will allow to 
# reproduce the tsv file in AERSMine
dat<-read.table("aersMineExploreDataSet_4372 additional renal ADRS.tsv",skip=5,header=TRUE,sep="\t")
...
# Clean up dataset by removing unncessary comma's
...
# Figure all tubular ADRs - Supplemental
pdf("figure_Supplemental_all_Tubular_ADRs.pdf", width=10,height=8)
...
pdf("figure_1_Nephropathies.pdf")
ggplot(data=resNP)+
...
# Figure Tubular disorders - Supplemental
...
pdf("figure_Supplemental_TubularDisorder.pdf")
ggplot(data=resTUB)+
  geom_point(aes(x=drug,y=ror))+
  theme_classic()+
  geom_errorbar(aes(x=drug,ymin=CIdown,ymax=CIup))+
...
pdf("figure_Supplemental_Glomerulonephritis_Nephrotic syndrome.pdf")
ggplot(data=resGN)+
...
### Figure Glomerulonephritis vs Hypertension
...
pdf("figure_1_GlomNeph_vs_Hypertension_color.pdf")
...
pdf("figure_1_GlomNeph_vs_Hypertension_col.pdf")
...
# Figure Tubular vs Nephropathies
...
# Write numbers for RORs to tables
write.csv(x=resGN,file="table_GlomNep.csv",quote=F,row.names = F)
write.csv(x=resNP,file="table_Nephropath.csv",quote=F,row.names = F)
write.csv(x=resTD,file="table_TubularDysfun.csv",quote=F,row.names = F)
